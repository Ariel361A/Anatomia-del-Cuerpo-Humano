...
extern int iniciarAppModelo();   // Modelo 1: Esqueleto
extern int iniciarAppModelo2();  // Modelo 2: Órganos Internos
...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        float botonAncho = display_w * 0.35f;
        ...
        if (ImGui::Button("Esqueleto Humano", ImVec2(botonAncho, botonAlto))) {
            iniciarModelo1 = true;
        }

        ImGui::Dummy(ImVec2(0.0f, display_h * 0.03f));
        ImGui::SetCursorPosX((display_w - botonAncho) * 0.5f);
        if (ImGui::Button("Órganos Internos", ImVec2(botonAncho, botonAlto))) {
            iniciarModelo2 = true;
        }
        ...
        // Verificamos cuál botón fue presionado y cerramos esta ventana para abrir el modelo correspondiente
        if (iniciarModelo1 || iniciarModelo2 )
            break;
    }

    // Shutdown ImGui y GLFW antes de abrir el modelo
    ...
    else if (iniciarModelo2) {
        return iniciarAppModelo2();
    }
